...
public class TestimonialAdminController {
    ...
    @PostMapping("/create")
    public String createTestimonial(Model model, @ModelAttribute("testimonial") TestimonialCreateDto testimonial) {
        model.addAttribute("students", studentRepository.findAll()
                .stream().filter(s -> !s.isDeleted()).map(dto -> mapper.map(dto, Student.class)));
        testimonialRepository.save(mapper.map(testimonial, Testimonial.class));
